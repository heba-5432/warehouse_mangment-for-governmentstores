...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('catogery.store')}}">
								@csrf
								@method('POST')
...
@if(!empty('$Catogery'))
  ...
  <th width="1%"><input type="checkbox" id="select-all" onclick="toggleSelectAll(this)">all</th>
                                        ...
                                        @foreach ($Catogery as $cat_items )

                                        <td width="1%"><input type="checkbox" class="table-checkbox" name="itemselected[]" value=""></td>
                                        <td width="1%">{{$i=$i+1}}</td>
												<td width="1%" >{{$cat_items->ar_title}}</td>
                                                <td width="1% " >{{$cat_items->En_title}}</td>
                                                <td width="1% " >{{$cat_items->description}}</td>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="POST" action="{{route('catogery.edit',$cat_items->id)}}">

					@csrf
								@method('POST')
                                <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ar" placeholder=" item title arabic" value="{{$cat_items->ar_title}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_eng" placeholder=" item title in english" value="{{$cat_items->En_title}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrption" placeholder="descrption" value="{{$cat_items->description}}">
									</div>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						<a href="{{route('catogery.destroy',$cat_items->id)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
		...
		<!-- End Basic modal - delete-alert end---->
                                                ...
                                                <td><a class="btn ripple btn-primary" href="{{route('item.add',$cat_items->id)}}">add item</a></td>
											</tr>
@endforeach
@endif
										</tbody>
