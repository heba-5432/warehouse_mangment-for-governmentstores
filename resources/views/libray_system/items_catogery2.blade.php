					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							<div class="card-header">
								...
								<p class="mb-2" >ادخل تصنيف فرعى (مثال اكشن - دراما )</p></h4>
							</div>
...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('catogery2.store')}}">
								@csrf
								@method('POST')
...
@if(!empty('$Catogery2'))
  ...
  <th><input type="checkbox" id="select-all" onclick="toggleSelectAll(this)">all</th>
                                        ...
                                        <td width="1%">{{$i=$i+1}}</td>
												<td width="1%" >{{$cat2->ar_title}}</td>
                                                <td width="1% " >{{$cat2->En_title}}</td>
                                                <td width="1% " >{{$cat2->description}}</td>

												<td width="">
													<!-- Basic modal -edit button--start--->
													<a class="btn btn-sm btn-info" data-target="#mo{{$cat2->id}}" data-toggle="modal" href="{{$cat2->id}}"><i style="padding:8px;" class="las la-pen"></i></a>
                                                   <!-- Basic modal -edit alert--start--->
<div class="modal" id="mo{{$cat2->id}}">
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="POST" action="{{route('catogery2.edit',$cat2->id)}}">

					@csrf
								@method('POST')
                                <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ar" placeholder=" item title arabic" value="{{$cat2->ar_title}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_eng" placeholder=" item title in english" value="{{$cat2->En_title}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrption" placeholder="descrption" value="{{$cat2->description}}">
									</div>
 ...
 <!-- Basic modal -delete button--start--->
  <a class="btn ripple btn-danger" data-target="#mod{{$cat2->id}}" data-toggle="modal" href="{{$cat2->id}}"><i  class="las la-trash"></i></a>
					<!-- Basic modal -delete button--end--->
<!-- Basic modal -delete alert--start--->
<div class="modal" id="mod{{$cat2->id}}">
<div class="modal-dialog modal-lg" role="document">
<div class="modal-content modal-content-demo">
					<div class="modal-header">
					...
					<div class="modal-footer">
						<a href="{{route('catogery2.destroy',$cat2->id)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
		...
		<!-- End Basic modal - delete-alert end---->
                                                ...
                                                <td><!--a class="btn ripple btn-primary" href="{{route('item.add',$cat2->id)}}">add item</a--></td>
											</tr>
@endforeach
@endif
										</tbody>
