					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							<div class="card-header">
								...
								<p class="mb-2">ادخل  تصنيف جديد</p></h4>
							</div>
...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('items_store_catogery.store')}}">
								@csrf
								@method('POST')
									<div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_arc" placeholder=" item title arabic" value="{{old('title_arc')}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engc" placeholder=" item title in english" value="{{old('title_engc')}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptionc" placeholder="descrption"  value="{{old('descrptionc')}}">
									</div>
...
<th></th>
                                        ...
                                        @foreach ($ItemsCatogery as $cat_items )

                                        <td width="1%"><input type="checkbox" class="table-checkbox" name="itemselected[]" value=""></td>
                                        <td width="1%">{{$i=$i+1}}</td>
												<td width="1%" >{{$cat_items->ar_titlec}}</td>
                                                <td width="1% " >{{$cat_items->En_titlec}}</td>
                                                <td width="1% " >{{$cat_items->descriptionc}}</td>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="post" action="{{route('items_store_catogery.edit',$cat_items->id)}}">

					@csrf
								@method('POST')
                                <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_arc" placeholder=" item title arabic" value="{{$cat_items->ar_titlec}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engc" placeholder=" item title in english" value="{{$cat_items->En_titlec}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptionc" placeholder="descrption" value="{{$cat_items->descriptionc}}">
									</div>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						<a href="{{route('items_store_catogery.destroy',$cat_items->id)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
