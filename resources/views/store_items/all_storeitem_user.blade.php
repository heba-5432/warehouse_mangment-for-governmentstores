...
<input input type="number" id="price" name="used_price" min="0" step="0.01" placeholder="0.00"  value="">
<label>يتم الحساب من ضمن  قيمه السعريه للمخزن</label>
<select name=" still_exitu">
...
<select class=" select2 col-lg-6 mg-t-20 mg-lg-t-0 " name="store_items" style="width:30%">
    ...
    @if(!empty($all_stores_items))
    @foreach ($all_stores_items as $store_items)
<option value="{{$ur_item-> storeitid}}"
@if(!empty($ur_item->item_store_id))
@if ($ur_item->item_store_id ==$store_items->storeitid) ? {{$ur_item-> storeitid}} selected :''

@endif

@endif

>{{$ur_item->title_arai}}</option>
@endforeach
@endif
</select>
...
<label style ="margin:2px; padding:3px;" >العدد المسترجع</label>
 ...
 <input  style ="width:120px;"  onchange="calculateSum" type="number" class="col-lg-5 mg-t-20 mg-lg-t-0" id="num2" name ="returned_number" placeholder=" العدد الكلى بالقطعه" value=" {{ $ur_item->number_returnedis}}" min="1" required>
