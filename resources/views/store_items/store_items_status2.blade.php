					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							<div class="card-header">
								...
								<p class="mb-2"> ادخل حاله المنتج (دائم -مستهلك)</p></h4>
							</div>
...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('items_store_status2.store')}}">
								@csrf
								@method('POST')
									<div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ars2" placeholder=" item title arabic" value="{{old('title_arc')}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engs2" placeholder=" item title in english" value="{{old('title_engc')}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptions2" placeholder="descrption"  value="{{old('descrptionc')}}">
									</div>
...
@if(!empty('$Itemsstat1'))
  ...
  <th width="1%"><input type="checkbox" id="select-all" onclick="toggleSelectAll(this)">all</th>
                                        ...
                                        @foreach ($Itemsstat2 as $cat_items )

                                        <td width="1%"><input type="checkbox" class="table-checkbox" name="itemselected[]" value=""></td>
                                        <td width="1%">{{$i=$i+1}}</td>
												<td width="1%" >{{$cat_items->ar_titles2}}</td>
                                                <td width="1% " >{{$cat_items->En_titles2}}</td>
                                                <td width="1% " >{{$cat_items->descriptions2}}</td>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="post" action="{{route('items_store_status2.edit',$cat_items->id)}}">

					@csrf
								@method('POST')
                                <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ars2" placeholder=" item title arabic" value="{{$cat_items->ar_titles2}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engs2" placeholder=" item title in english" value="{{$cat_items->En_titles2}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptions2" placeholder="descrption" value="{{$cat_items->descriptions2}}">
									</div>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						<a href="{{route('items_store_status2.destroy',$cat_items->id)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
