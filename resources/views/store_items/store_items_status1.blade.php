					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							<div class="card-header">
								...
								<p class="mb-2">ادخل حاله (جديد - كهنه -مستعمل)</p></h4>
							</div>
...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('items_store_status1.store')}}">
								@csrf
								@method('POST')
									<div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ars" placeholder=" item title arabic" value="{{old('title_ars')}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engs" placeholder=" item title in english" value="{{old('title_engs')}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptions" placeholder="descrption"  value="{{old('descrptions')}}">
									</div>
...
@if(!empty('$Itemsstat1'))
  ...
  <th width="1%"><input type="checkbox" id="select-all" onclick="toggleSelectAll(this)">all</th>
                                            <th width="1%">no.&nbsp;&nbsp;</th>
                                                ...
                                                <th class="border-bottom-0"  width="1%">English title &nbsp;&nbsp;</th>
                                                <th class="border-bottom-0"  width="1%"> {{trans('transfile.description')}} &nbsp;&nbsp;</th>

												<th class="border-bottom-0" width="1% auto" colspan="2" >action</th>

                                                <th class=""  width="1% " >add item</th>

										</thead>
                                </tr>
										<tbody>

                                        @foreach ($Itemsstat1 as $cat_items )

                                        <td width="1%"><input type="checkbox" class="table-checkbox" name="itemselected[]" value=""></td>
                                        <td width="1%">{{$i=$i+1}}</td>
												<td width="1%" >{{$cat_items->ar_titles}}</td>
                                                <td width="1% " >{{$cat_items->En_titles}}</td>
                                                <td width="1% " >{{$cat_items->descriptions}}</td>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="post" action="{{route('items_store_status1.edit',$cat_items->id)}}">

					@csrf
								@method('POST')
                                <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_ars" placeholder=" item title arabic" value="{{$cat_items->ar_titles}}">
									</div>
                                    <div class="form-group">
										<input type="text" class="form-control" id="inputName"  name ="title_engs" placeholder=" item title in english" value="{{$cat_items->En_titles}}">
									</div>

									<div class="form-group">
										<input type="text" class="form-control" id="inputName" name ="descrptions" placeholder="descrption" value="{{$cat_items->descriptions}}">
									</div>
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						<a href="{{route('items_store_status1.destroy',$cat_items->id)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
