					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							<div class="card-header">
								...
								<p class="mb-2">email status info</p></h2>
							</div>
...
@endif
								...
								<form class="form-horizontal"  method="POST" action="{{route('emails_report.ser')}}">
								@csrf
								@method('POST')
...
<div class="form-group col-lg-12 mg-t-20 mg-lg-t-0" >
										...
										<input type="date" class=" col-lg-4 mg-t-20 mg-lg-t-0" id="inputName"  name ="start_date" placeholder="start date" value="{{old('start_date')}}">

                                        <label>end date</label>
										<input type="date" class=" col-lg-4 mg-t-20 mg-lg-t-0" id="inputName"  name ="end_date" placeholder="start date" value="{{old('end_date')}}">
...
<!---div----->
  ...
  <a class="btn ripple btn-primary" data-target="#mod" data-toggle="modal" href="{{route('emails_report2.destroy2')}}">delete all</a>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						<a href="{{route('emails_report2.destroy2')}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
		</div>
		<!-- End Basic modal - delete-alert end----><a href=""> <button class="btn ripple btn-secondary"  type="button">show all</button></a>
...
<th class="border-bottom-0" width="50px" >action</th>
                                        ...
                                        @foreach($users_aw as $userrew)
											...
											<td>{{$i=$i+1}}</td>
                                            <td>{{$userrew->name}}</td>
												<td width="5%" > {{$userrew->emuser_national }}</td>
<td>{{$userrew->email}}</td>
<td>{{$userrew->emdate }}</td>
<td>



@if($userrew->emstatus  == "sent")
<span style="color:green">{{$userrew->emstatus}}</span>
@endif
@if($userrew->emstatus  != "sent")
<span style="color:red">{{$userrew->emstatus}}</span>
@endif
...
<div class="modal" id="mo{{$userrew-> emuser_id1}}">
...
<div class="modal-content modal-content-demo">
					...
					<div class="modal-body">
						...
						<form class="form-horizontal"  method="POST" action="{{route('emails_report.edit',$userrew-> emuser_id1)}}">
...
<td>
 ...
 <!-- Basic modal -delete button--start--->
  <a class="btn ripple btn-primary" data-target="#mod{{$userrew-> emuser_id1}}" data-toggle="modal" href="">delete</a>
					<!-- Basic modal -delete button--end--->
<!-- Basic modal -delete alert--start--->
<div class="modal" id="mod{{$userrew-> emuser_id1}}">
			<div class="modal-dialog" role="document">
				<div class="modal-content modal-content-demo">
					<div class="modal-header">
						<h6 class="modal-title">delete confirm</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
					...
					<div class="modal-footer">
						<a href="{{route('emails_report.destroy',$userrew-> emuser_id1)}}"><button class="btn ripple btn-primary" type="button">confirm delete</button></a>
						<button class="btn ripple btn-secondary" data-dismiss="modal" type="button">cancel</button>
					</div>
				</div>
			</div>
